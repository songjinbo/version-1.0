...
//未用到的两个函数
double Star::RandomGenerate(long * idum)
{
...
extern vector<double> voxel_x; //本线程的输入变量,GetVoxelTread的输出
...
bool Star::Creatgraph()
{
	...
	for (i=0; i<3; i++)
	{
		pos[i] = (int)((endRealPosition[i] - minValue[i])/sizeofBlock + 0.5);
	}
	endp   = nodes[pos[2]][pos[1]][pos[0]];			/// 设置结束节点
	...
	if ( node != NULL  && endp.getWalkable() && (*node == endp))
	{
		...
		if ((fp = fopen(outputfile2_3DAStar.c_str(), "w"))!= NULL) //全局变量5
		{
			for ( int i = path.size()-1 ; i >=0  ; --i )
			{
				...
				path_x.push_back(path[i].realx);
				path_y.push_back(path[i].realy);
				path_z.push_back(path[i].realz);
			}
			fclose(fp);
		}
