...
extern CCriticalSection critical_single_rawdata;//与主线程display_image函数的接口
...
char save_file[MAX_PATH] = { "./data/depth.log" }; //需要和下面的logFilename变量一致
	...
	for (OcTree::leaf_iterator it = tree->begin(), end = tree->end(); it != end; ++it) {
		...
		if (tree->isNodeOccupied(*it)){
			...
			if (size == res)
			{
				...
				voxel_x.push_back(int(it.getX() * 10) / 10.0); //it.getX()保存到文件再读入变量中后的值不等于it.getX(),而是等于int(it.getX()*10)/10.0
